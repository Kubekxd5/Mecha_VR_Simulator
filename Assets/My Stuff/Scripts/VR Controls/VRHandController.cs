...
public class VRHandController : MonoBehaviour
{
    ...
    public void SendHapticImpulse(float amplitude = 0.5f, float duration = 0.1f)
    {
        if (grabAction.action.controls.Count > 0)
        {
            InputDevice device = grabAction.action.controls[0].device;

            if (device != null && device is XRController xrController)
            {
                var command = UnityEngine.InputSystem.XR.Haptics.SendHapticImpulseCommand.Create(
                    motorChannel: 0,
                    motorAmplitude: amplitude,
