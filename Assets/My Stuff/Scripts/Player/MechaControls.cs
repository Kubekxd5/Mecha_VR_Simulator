...
public class MechaControls : MonoBehaviour
{
    ...
    private MechaRuntimeData runtimeData;
    ...
    public void RegisterJoystick(VRJoystickController joystick)
    {
        if (joystick.isLeft) movementJoystick = joystick;
        else aimingJoystick = joystick;

        Debug.Log($"MechaControls: Registered {(joystick.isLeft ? "Left" : "Right")} Joystick.");
    }

    public void InitializeControls()
    {
        runtimeData = GetComponent<MechaRuntimeData>();
    ...
    public void MoveHeightUp()
    {
        desiredBodyHeight += Time.deltaTime * heightInputSpeed;
    }
    public void MoveHeightDown()
    {
        desiredBodyHeight -= Time.deltaTime * heightInputSpeed;
    }
    ...
    private void HandleFiring()
    {
        ...
        // LEFT SIDE
        if ((movementJoystick != null && movementJoystick.IsTriggerPressed) || Input.GetKey(KeyCode.Alpha1))
        {
            FireWeaponsBySide(true);
            if (movementJoystick.isHeld) movementJoystick.HoldingHand.SendHapticImpulse(0.5f, 0.1f);
        }

        // RIGHT SIDE
        if ((aimingJoystick != null && aimingJoystick.IsTriggerPressed) || Input.GetKey(KeyCode.Alpha1))
        {
            FireWeaponsBySide(false);
            if (aimingJoystick.isHeld) aimingJoystick.HoldingHand.SendHapticImpulse(0.5f, 0.1f);
        }
    }

    private void FireWeaponsBySide(bool fireLeftSide)
    {
        foreach (var weapon in runtimeData.ActiveWeaponScripts)
        {
            if (weapon == null) continue;

            if (weapon.IsLeft == fireLeftSide)
            {
                weapon.Fire();
            }
        }
    }
    private void FireWeaponsByClass(WeaponClass targetClass)
    {
        foreach (var weapon in runtimeData.ActiveWeaponScripts)
        {
            if (weapon.WeaponData.weaponClass == targetClass)
            {
                weapon.Fire();
            }
        }
