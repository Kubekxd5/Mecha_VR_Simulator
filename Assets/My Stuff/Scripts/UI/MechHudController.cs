...
public class MechHudController : MonoBehaviour
{
    ...
    private void OnDisable()
    {
        ...
        {
            mechaRuntimeData.OnHealthChanged -= UpdateHealthDisplay;
        }
    }
    ...
    public void Initialize(Transform mechaTransform)
    {
        ...
        if (this.mechaRuntimeData != null)
        {
            this.mechaRuntimeData.OnHealthChanged += UpdateHealthDisplay;

            UpdateHealthDisplay(this.mechaRuntimeData.CurrentHealth, this.mechaRuntimeData.MechaData.mechaStats.health);
        }
    ...
    private void UpdateHealthDisplay(float current, float max)
    {
        ...
        // Update Text
        healthLabel.text = $"HP: {percentInt}%";

        // Change color based on health
        if (percent > 0.6f)
            healthLabel.style.color = new StyleColor(new Color(0f, 1f, 0.86f));
        else if (percent > 0.3f)
            healthLabel.style.color = new StyleColor(Color.yellow);
        else
            healthLabel.style.color = new StyleColor(Color.red);
    }
